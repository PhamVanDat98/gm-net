...
**Ghi chú triển khai M5** ([ĐỀ XUẤT] đã cụ thể hóa — `InterpolationBuffer`):

- **Đồng hồ render neo theo dòng snapshot** (stream clock), KHÔNG theo
  `ClockSync.serverTickNow`: ước lượng của ClockSync đứng ở "hiện tại server" (+RTT/2),
  nếu lấy nó trừ delay 100ms thì với RTT 200ms buffer cạn vĩnh viễn (snapshot mới nhất đã
  về chỉ tới `serverTickNow − RTT/2`). Thứ interpolation cần là "tick mới nhất ĐÃ VỀ TỚI
  client" trôi đều theo thời gian thực: `streamTick(now) = now/tickMs + offset`, `offset`
  bám max theo gói ít bị trễ nhất và rỉ chậm xuống (EWMA 0.02) để hấp thụ drift.
  `renderTick = streamTick(now) − delay/tickMs` — delay 100ms khi đó đo đúng "đệm chống
  jitter/loss" bất kể RTT.
- Adaptive delay hai biến: `targetDelay` nhảy theo sự kiện (cạn buffer → +15ms, trần
  200ms; sạch → rỉ về 100ms ~2ms/s), `delay` hiệu dụng trượt về target ≤ ~100ms delay/s
  thực (vài ms mỗi frame) — không time-warp.
- Extrapolate quá mẫu cuối theo velocity cuối, cap 2 tick; quá cap → freeze **tại vị trí
  cap** (giữ liên tục, không bật ngược về mẫu cuối). Entity vắng mặt khỏi stream (despawn)
  gỡ track khi render time đã đi qua hết lịch sử của nó + grace 2 tick.
- **`GameSession`** (task 5.2) là lớp ghép mặc định phía client: GameClient +
  PredictionWorld/Reconciler (local) + InterpolationBuffer (remote, loại local entity
  khỏi stream) + TransformSmoother; game gọi `tick(payload)` mỗi tick sim và
  `getRenderState(now)` mỗi frame render, `hud(now)` gộp metrics §8.
...
**Ghi chú M5:** lệch protocol đã xảy ra đúng chỗ dự đoán, ở bước **matchmake HTTP** (server
0.17 trả reservation phẳng `{name, roomId, processId, sessionId}`, client 0.16 mong
`{room: {...}, sessionId}`); không có colyseus.js 0.17 để ghim nên `connectGameRoom` tự
POST matchmake rồi reshape và đưa vào `consumeSeatReservation` — đường WS + room protocol
hai bản vẫn khớp (e2e demo-2d qua socket thật xác nhận).
